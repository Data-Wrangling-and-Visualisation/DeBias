...
function createSandboxNetwork(options) {
  ...
  d3.json(dataUrl)
    .then((rawData) => {
      ...
      filteredArticles.forEach((article) => {
        ...
        nodeEntry.nerTypes.set(
          article.keyword.keyword.type,
          (nodeEntry.nerTypes.get(article.keyword.keyword.type) || 0) + mention
        );

        // Increment count for each valid NEWS TOPIC of the article this keyword appeared in
        article.topics.forEach((topic) => {
          nodeEntry.topics.set(
            topic.text,
            (nodeEntry.topics.get(topic.text) || 0) + 1
          );
        });
        ...
        article.keyword.related.forEach((pair) => {
          ...
          linksCount
            ...
            .set(
              id2,
              (linksCount.get(id1).get(id2) || 0) + pair.cooccurrence_count
            );
        });
      });
        ...
        targets.forEach((count, targetLowerId) => {
          if (count >= edgeThreshold) {
            ...
            if (sourceNode && targetNode) {
              ...
              console.warn(
                `Node not found for link processing: ${sourceLowerId} or ${targetLowerId}`
              );
            }
          }
        });
      ...
      if (links.length === 0) {
        displayMessage(
          "No keyword connections found with current filters/threshold."
        );
        return;
      }
      ...
      const allNerTypes = new Set(
        finalNodes.flatMap((node) => Array.from(node.nerTypes.keys()))
      );
      const nerColorScale = d3
        .scaleOrdinal(d3.schemeCategory10)
        .domain(Array.from(allNerTypes).sort());

      // News Topic Color Scale (for tooltip)
      const topicColorScale = d3
        .scaleOrdinal(d3.schemeTableau10)
        .domain(NEWS_CATEGORIES);
      ...
      const simulation = d3
          ...
          d3
            ...
            .distance((d) =>
              Math.max(
                ...
                130 -
                  d.value * 3 -
                  (calculateNodeRadius(d.source) +
                    calculateNodeRadius(d.target)) /
                    2
              )
            )
        ...
        .force(
          "charge",
          d3.forceManyBody().strength(-90 - finalNodes.length * 1.8)
        )
        ...
        .force(
          ...
          d3
            .forceCollide()
            .radius((d) => calculateNodeRadius(d) + 5)
            .strength(0.9)
        );
      ...
      const link = linkGroup
        .selectAll("line")
        .data(finalLinks, (d) => `${d.source.id}-${d.target.id}`)
        .join("line")
        .attr("stroke", "#ccc")
      ...
      node.each(function (d) {
        ...
        if (d.nerTypes.size <= 1) {
          // Treat 0 or 1 NER type as a single circle
          const nerType =
            d.nerTypes.size === 1 ? d.nerTypes.keys().next().value : "UNKNOWN";
          group
            .append("circle")
            .attr("r", radius)
      ...
      node
        .on("mouseover", function (event, d) {
          ...
          if (d.nerTypes.size === 0) typeHtml = "Type: N/A";
            ...
            typeHtml = `Type: <span style="font-weight: 500; color: ${nerColorScale(
              nerType
            )};">${nerType}</span>`;
          } else {
            typeHtml =
              'Types:<ul style="margin: 3px 0 0 15px; padding: 0; font-size: 0.9em; list-style-type: none;">';
            const sortedTypes = Array.from(d.nerTypes.entries()).sort(
              ([, countA], [, countB]) => countB - countA
            );
            sortedTypes.forEach(([type, count]) => {
              typeHtml += `<li style="margin-bottom: 2px;"><span style="display: inline-block; width: 10px; height: 10px; background-color: ${nerColorScale(
                type
              )}; border-radius: 2px; margin-right: 5px;"></span>${type}: ${count}</li>`;
            });
            typeHtml += "</ul>";
          }
          ...
          if (d.topics.size > 0) {
            topicHtml =
              '<div style="margin-top: 8px; padding-top: 5px; border-top: 1px solid #eee; font-size: 0.9em;">Topic Distribution:<ul>';
            const sortedTopics = Array.from(d.topics.entries()).sort(
              ([, countA], [, countB]) => countB - countA
            );
            sortedTopics.forEach(([topic, count]) => {
              topicHtml += `<li style="margin-bottom: 2px; list-style-type: none;"><span style="display: inline-block; width: 10px; height: 10px; background-color: ${topicColorScale(
                topic
              )}; border-radius: 50%; margin-right: 5px;"></span>${topic}: ${count}</li>`;
            });
            topicHtml += "</ul></div>";
          } else {
            topicHtml =
              '<div style="margin-top: 5px; font-size: 0.9em; color: #888;">No topic data</div>';
          }
          ...
          link
            ...
            .attr("stroke", (l) =>
              l.source.id === d.id || l.target.id === d.id ? "#777" : "#eee"
            );
        })
      ...
      const zoom = d3
        ...
        .on("zoom", (event) => {
          ...
          svg
            ...
            .style(
              "font-size",
              `${Math.min(12, Math.max(6, 9 / Math.sqrt(scale)))}px`
            );
        });
  ...
  function applyDragHandlers(selection) {
    ...
    function dragended(event, d) {
      if (!event.active)
        simulation.alphaTarget(0); /* Keep fixed: d.fx = null; d.fy = null; */
    }
    selection.call(
      d3
        .drag()
  ...
  function showPublicationModal(nodeData) {
    ...
    if (articlesToShow.length > maxShown)
      modalFooter.text(
        `Showing first ${maxShown} of ${articlesToShow.length} articles (alphabetical).`
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const edgeThresholdValueSpan = document.getElementById(
  ...
  function getSandboxSettings() {
    const selectedTopicOptions = Array.from(topicSelector.selectedOptions).map(
      (opt) => opt.value
    );
    ...
    if (
      ...
      !(selectedTopicOptions.length === 1 && selectedTopicOptions[0] === "all")
    ) {
    ...
    return {
      containerSelector: "#sandbox-preview",
      dataUrl: input_link,
      startDate: startDateInput.value || null,
      endDate: endDateInput.value || null,
      selectedTopics: topicsToFilter, // Pass the cleaned filter array
      maxNodes: parseInt(nodeCountSlider.value, 10),
  ...
  if (!document.getElementById("sandbox-tooltip")) {
    ...
    sandboxTooltip.id = "sandbox-tooltip";
    sandboxTooltip.className = "tooltip";
    sandboxTooltip.style.opacity = "0";
    sandboxTooltip.style.position = "absolute";
    sandboxTooltip.style.pointerEvents = "none";
    document.body.appendChild(sandboxTooltip);
  }
  if (!document.getElementById("publication-modal")) {
    console.error(
      "Publication modal HTML not found! Article list on click will not work."
    );
  }
